      ...
      <div className="max-w-6xl w-full space-y-12 py-12 animate-in fade-in zoom-in duration-1000">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 bg-[var(--bg-surface)] p-10 rounded-[48px] border border-[var(--border-subtle)] shadow-2xl">
          <div className="flex items-center gap-8">
            ...
            <div>
              ...
              <p className="text-[var(--text-muted)] text-xs font-bold uppercase tracking-[0.3em] mt-2">
          ...
          <button 
            onClick={logout}
            className="flex items-center gap-4 px-8 py-4 bg-[var(--bg-deep)]/40 hover:bg-red-500/10 text-[var(--text-muted)] hover:text-red-400 rounded-[24px] transition-all border border-[var(--border-subtle)] active:scale-95 group"
          >
        ...
        {loading && projects.length === 0 ? (
          <div className="h-96 flex flex-col items-center justify-center space-y-6">
            <div className="p-6 bg-[var(--accent-soft)] rounded-3xl border border-[var(--accent)]/20 animate-pulse">
                <Loader2 className="w-10 h-10 text-[var(--accent)] animate-spin" />
            </div>
            <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-[var(--accent)]/50 animate-pulse">Sincronizzazione Indice...</p>
          </div>
        ) : (
        ...
        {projects.length === 0 && !loading && (
          <div className="text-center py-24 bg-[var(--bg-surface)]/10 rounded-[48px] border border-dashed border-[var(--border-subtle)] animate-in fade-in duration-1000">
            <PlusCircle className="w-16 h-16 text-[var(--text-muted)] mx-auto mb-6 opacity-20" />
            <div className="space-y-2">
                <p className="text-xl font-medium text-[var(--text-secondary)] tracking-tight">Il manoscritto è ancora una pagina bianca</p>
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--text-muted)]">Clicca su 'Nuova Opera' per iniziare a plasmare la tua storia</p>
            </div>
          </div>
        )}
      </div>
